...
(String, String) analyseStrength(WidgetRef ref) {
  ...
  debugPrint("irca $strengthDelta $irca");
  return (grades[ircaMin.clamp(1, 30)], grades[ircaMax.clamp(1, 30)]);
}

(String, String) analyseRfd(WidgetRef ref) {
...
//TODO Mean or Peak????
  ...
  int ircaMin = irca.round().toInt() - 1;
  int ircaMax = irca.round().toInt() + 1;
  return (grades[ircaMin.clamp(1, 30)], grades[ircaMax.clamp(1, 30)]);
}

(String, String) analyseCft(WidgetRef ref) {
  User user = ref.watch(userProvider);
  ...
  int ircaMin =
      (cftTests.asymptoticForce / user.weight * 100 * 0.25 + 6).round().toInt();
  int ircaMax =
      (cftTests.asymptoticForce / user.weight * 100 * 0.35 + 6).round().toInt();
